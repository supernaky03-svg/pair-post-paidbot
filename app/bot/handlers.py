...
async def _cleanup_temp_target_if_needed(state: FSMContext, user_id: int) -> None:
    data = await state.get_data()
    temp_target_input = data.get("temp_target_input")
    if not temp_target_input or data.get("temp_target_committed"):
        return

    committed_pairs = await pair_repo.list_for_user(user_id)
    committed_keys = {p.target_key for p in committed_pairs if getattr(p, "target_key", None)}
    try:
        await pair_service.targets.cleanup_temporary_target(temp_target_input, committed_keys)
    except Exception:
        pass

    await state.update_data(
        temp_target_input=None,
        temp_target_key=None,
        temp_target_chat_id=None,
        temp_target_title=None,
        temp_target_joined=False,
...
async def _go_back(target: Message | CallbackQuery, state: FSMContext, language: str, user_id: int) -> None:
    ...
    await _cleanup_temp_target_if_needed(state, user_id)
...
async def _ensure_timeout(target: Message | CallbackQuery, state: FSMContext, language: str) -> bool:
    ...
    if last_activity and (_now_ts() - float(last_activity)) > settings.flow_timeout_minutes * 60:
        await _cleanup_temp_target_if_needed(state, target.from_user.id)
        await state.clear()
...
async def callback_router(call: CallbackQuery, state: FSMContext) -> None:
    ...
    if (
        data.startswith("set_forward:")
        or data.startswith("set_post:")
        or data.startswith("set_remove_url:")
    ) and current_state == RuleStates.waiting_value.state:
        action = data.split(":", 1)[1]
        info = await state.get_data()
    ...
    if current_state == EditSourceStates.waiting_scan.state:
        ...
        try:
            ...
            await _show_step(
                message,
                state,
                t(language, "invalid_scan"),
                reply_markup=text_step_keyboard("flow", language),
            )
            return
