...
async def _remove_main_menu(target: Message | CallbackQuery) -> None:
    await _send(target, "⁣", reply_markup=hide_reply_keyboard())


async def _idle_markup_for_user(user_id: int, language: str):
    user = await user_repo.get(user_id)
...
async def _go_back(target: Message | CallbackQuery, state: FSMContext, language: str, user_id: int) -> None:
    ...
    if not history:
        await state.clear()
        await _send(target, t(language, "cancelled"), reply_markup=await _idle_markup_for_user(user_id, language))
        return
    ...
    await _send(
        target,
        t(language, previous.get("prompt_key") or previous["current_prompt_key"], **previous.get("prompt_fmt", {})),
        reply_markup=markup,
    )
...
async def _ensure_timeout(target: Message | CallbackQuery, state: FSMContext, language: str) -> bool:
    ...
    if last_activity and (_now_ts() - float(last_activity)) > settings.flow_timeout_minutes * 60:
        await state.clear()
        await _send(target, t(language, "flow_timeout"), reply_markup=await _idle_markup_for_user(target.from_user.id, language))
        return False
...
async def _ensure_access_message(message: Message, state: FSMContext):
    ...
    if user.status != "activated":
        ...
        await message.answer(reason, reply_markup=language_keyboard())
    ...
    await message.answer(reason, reply_markup=main_menu(_lang(user)))
    return None
...
async def _ensure_access_callback(call: CallbackQuery, state: FSMContext):
    ...
    if user.status != "activated":
        ...
        await call.message.answer(reason, reply_markup=language_keyboard())
        await state.set_state(OtpStates.waiting_otp)
        await state.update_data(last_activity=_now_ts(), current_prompt_key="otp_required", current_markup_payload={"type": "language"}, history=[])
        await call.answer()
        return None

    await call.message.answer(reason, reply_markup=main_menu(_lang(user)))
    await call.answer()
    return None
...
async def start_cmd(message: Message, state: FSMContext) -> None:
    ...
    if user.is_banned:
        await message.answer(t(language, "access_blocked"), reply_markup=hide_reply_keyboard())
        return
...
async def callback_router(call: CallbackQuery, state: FSMContext) -> None:
    ...
    if data.startswith("lang:"):
        ...
        await call.message.answer(
            t(new_language, "language_set"),
            reply_markup=main_menu(new_language) if is_activated else hide_reply_keyboard(),
        )
        state_data = await state.get_data()
    ...
    if data.startswith("kw_action:") and current_state == KeywordStates.waiting_action.state:
        ...
        if action == "set_ban":
            if pair.keyword_values:
                ...
                await call.message.answer(t(language, "keyword_send"), reply_markup=text_step_keyboard("kw_text", language))
            ...
            await call.message.answer(t(language, "keyword_remove"), reply_markup=text_step_keyboard("kw_text", language))
        await call.answer()
        return
    ...
    if data.startswith("ads_pair_add:") and current_state == AdsStates.waiting_pair_for_add.state:
        ...
        if choice.isdigit():
            ...
            await call.message.answer(t(language, "ads_send"), reply_markup=text_step_keyboard("ads_text", language))
        await call.answer()
        return
...
async def message_router(message: Message, state: FSMContext) -> None:
    ...
    if action == "add_pair":
        ...
        await message.answer(t(language, "pair_no_prompt"), reply_markup=text_step_keyboard("flow", language))
        return
    if action == "delete_pair":
        ...
        await message.answer(t(language, "send_pair_number"), reply_markup=text_step_keyboard("flow", language))
        return
    ...
    if current_state == AddPairStates.waiting_pair_no.state:
        ...
        await message.answer(t(language, "source_prompt"), reply_markup=text_step_keyboard("flow", language))
        return
    ...
    if current_state == AddPairStates.waiting_source.state:
        ...
        await message.answer(t(language, "scan_prompt"), reply_markup=text_step_keyboard("flow", language))
        return
    ...
    if current_state == AddPairStates.waiting_scan.state:
        ...
        await message.answer(t(language, "target_prompt"), reply_markup=text_step_keyboard("flow", language))
        return
    ...
    if current_state == AddPairStates.waiting_target.state:
        ...
        await message.answer(t(language, "ads_prompt"), reply_markup=text_step_keyboard("flow", language))
        return
    ...
    if current_state == EditSourceStates.waiting_pair_no.state:
        ...
        await message.answer(t(language, "send_new_source"), reply_markup=text_step_keyboard("flow", language))
        return
    ...
    if current_state == EditSourceStates.waiting_source.state:
        ...
        await message.answer(t(language, "send_scan_again"), reply_markup=text_step_keyboard("flow", language))
        return
    ...
    if current_state == EditTargetStates.waiting_pair_no.state:
        ...
        await message.answer(t(language, "send_new_target"), reply_markup=text_step_keyboard("flow", language))
        return
