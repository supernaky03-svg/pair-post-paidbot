...
async def _go_back(target: Message | CallbackQuery, state: FSMContext, language: str, user_id: int) -> None:
    ...
    if not history:
        await state.clear()
        await _send(target, t(language, "cancelled"), reply_markup=main_menu(language))
        return
    ...
    await state.update_data(
        history=history,
        current_prompt_key=previous.get("prompt_key"),
    ...
    await _send(target, t(language, previous["prompt_key"], **previous.get("prompt_fmt", {})), reply_markup=markup)


async def _cancel_flow(target: Message | CallbackQuery, state: FSMContext, language: str) -> None:
    await state.clear()
    await _send(target, t(language, "cancelled"), reply_markup=main_menu(language))
...
async def _ensure_timeout(target: Message | CallbackQuery, state: FSMContext, language: str) -> bool:
    ...
    if last_activity and (_now_ts() - float(last_activity)) > settings.flow_timeout_minutes * 60:
        await state.clear()
        await _send(target, t(language, "flow_timeout"), reply_markup=main_menu(language))
        return False
...
async def _ensure_access_message(message: Message, state: FSMContext):
    ...
    if allowed:
        return user
    await state.clear()
    await message.answer(reason, reply_markup=language_keyboard() if user.status != "activated" else main_menu(_lang(user)))
...
async def _ensure_access_callback(call: CallbackQuery, state: FSMContext):
    ...
    if allowed:
        return user
    await state.clear()
    await call.message.answer(reason, reply_markup=language_keyboard() if user.status != "activated" else main_menu(_lang(user)))
    if user.status != "activated":
        await state.set_state(OtpStates.waiting_otp)
        await state.update_data(last_activity=_now_ts(), current_prompt_key="otp_required", current_markup_payload={"type": "language"}, history=[])
    await call.answer()
...
async def callback_router(call: CallbackQuery, state: FSMContext) -> None:
    ...
    if data.startswith("lang:"):
        ...
        await call.message.answer(t(new_language, "language_set"), reply_markup=main_menu(new_language))
        ...
        if await state.get_state() and state_data.get("current_prompt_key"):
            ...
            markup = await _render_markup(call.from_user.id, new_language, state_data.get("current_markup_payload"))
            await call.message.answer(t(new_language, prompt_key, **prompt_fmt), reply_markup=markup)
        await call.answer()
        return
    ...
    if data.startswith("restore:"):
        ...
        if action == "fresh":
            ...
            await call.message.answer(t(language, "fresh_done"), reply_markup=main_menu(language))
        else:
            await call.message.answer(t(language, "restore_done"), reply_markup=main_menu(language))
    ...
    if data.endswith(":cancel") or data in {"kw_action:cancel", "ads_action:cancel"}:
        await _cancel_flow(call, state, language)
        await call.answer()
        return
    ...
    if data.startswith("add_post:") and current_state == AddPairStates.waiting_post_rule.state:
        ...
        await call.message.answer(t(language, "rule_forward_explain"), reply_markup=rule_keyboard("add_forward", language))
        await call.answer()
        return
    ...
    if data.startswith("add_forward:") and current_state == AddPairStates.waiting_forward_rule.state:
        ...
        await call.message.answer(summary, reply_markup=confirm_keyboard("pair_confirm", language))
        await call.answer()
        return
    ...
    if data.startswith("pair_confirm:") and current_state == AddPairStates.waiting_confirm.state:
        ...
        if action == "yes":
            ...
            try:
                ...
                await call.message.answer(f"Create failed: {exc}", reply_markup=main_menu(language))
            else:
                await call.message.answer(t(language, "pair_created"), reply_markup=main_menu(language))
            await state.clear()
        await call.answer()
        return
    ...
    if data.startswith("pair_delete:") and current_state == DeletePairStates.waiting_confirm.state:
        ...
        if action == "yes":
            ...
            try:
                ...
                await call.message.answer(f"Delete failed: {exc}", reply_markup=main_menu(language))
            else:
                await call.message.answer(t(language, "pair_deleted"), reply_markup=main_menu(language))
            await state.clear()
        await call.answer()
        return
    ...
    if data.startswith("edit_source_confirm:") and current_state == EditSourceStates.waiting_confirm.state:
        ...
        if action == "yes":
            ...
            try:
                ...
                await call.message.answer(f"Update failed: {exc}", reply_markup=main_menu(language))
            else:
                runtime_manager.clear_cache()
                await call.message.answer(t(language, "pair_updated"), reply_markup=main_menu(language))
            await state.clear()
        await call.answer()
        return
    ...
    if data.startswith("kw_pair:") and current_state == KeywordStates.waiting_pair.state:
        ...
        if choice.isdigit():
            ...
            if not pair or not pair.active:
                await call.message.answer(t(language, "pair_not_found"))
                await call.answer()
                return
    ...
    if data.startswith("kw_action:") and current_state == KeywordStates.waiting_action.state:
        ...
        if not pair:
            await call.message.answer(t(language, "pair_not_found"), reply_markup=main_menu(language))
            await state.clear()
            await call.answer()
            return
        if action == "set_ban":
            if pair.keyword_values:
                ...
                await call.message.answer(t(language, "keyword_updated"), reply_markup=main_menu(language))
                await state.clear()
            else:
                await state.update_data(pending_keyword_mode="ban")
                await _set_step(state, KeywordStates.waiting_add_values, prompt_key="keyword_send", markup_payload={"type": "main"})
                await call.message.answer(t(language, "keyword_send"), reply_markup=main_menu(language))
            ...
            await call.message.answer(t(language, "keyword_cleared"), reply_markup=main_menu(language))
            await state.clear()
        elif action == "add":
            ...
            await _set_step(state, KeywordStates.waiting_clear_values, prompt_key="keyword_remove", markup_payload={"type": "main"})
            await call.message.answer(t(language, "keyword_remove"), reply_markup=main_menu(language))
        await call.answer()
        return
    ...
    if data.startswith("ads_action:") and current_state == AdsStates.waiting_action.state:
        ...
        if action == "list":
            ...
            if not pairs:
                await call.message.answer(t(language, "ads_empty"), reply_markup=main_menu(language))
                await state.clear()
            else:
                await call.message.answer("\n\n".join(_pair_line(pair) for pair in pairs), reply_markup=main_menu(language))
                await state.clear()
        elif action == "add":
            await _set_step(
                state,
            ...
            await call.message.answer(t(language, "choose_pair"), reply_markup=pair_picker("ads_pair_add", pairs, language))
        elif action == "delete":
            pairs = [pair for pair in await pair_repo.list_for_user(call.from_user.id) if pair.ads]
            if not pairs:
                await call.message.answer(t(language, "ads_empty"), reply_markup=main_menu(language))
                ...
                await call.message.answer(t(language, "ads_delete_choose"), reply_markup=pair_picker("ads_pair_del", pairs, language))
        await call.answer()
        return
    ...
    if data.startswith("ads_pair_add:") and current_state == AdsStates.waiting_pair_for_add.state:
        ...
        if choice.isdigit():
            ...
            await _set_step(state, AdsStates.waiting_values, prompt_key="ads_send", markup_payload={"type": "main"})
            await call.message.answer(t(language, "ads_send"), reply_markup=main_menu(language))
        await call.answer()
        return
    ...
    if data.startswith("ads_pair_del:") and current_state == AdsStates.waiting_pair_for_delete.state:
        ...
        if choice.isdigit():
            ...
            if not pair:
                ...
                await call.message.answer(_pair_line(pair), reply_markup=confirm_keyboard("ads_delete_confirm", language))
        await call.answer()
        return
    ...
    if data.startswith("ads_delete_confirm:") and current_state == AdsStates.waiting_delete_confirm.state:
        ...
        if action == "yes":
            ...
            await call.message.answer(t(language, "ads_deleted"), reply_markup=main_menu(language))
            await state.clear()
        await call.answer()
        return
    ...
    if data.startswith("rule_pair:") and current_state == RuleStates.waiting_pair.state:
        ...
        if choice.isdigit():
            ...
            await call.message.answer(t(language, "rule_choose_value"), reply_markup=rule_keyboard(prefix, language))
        await call.answer()
        return
    ...
    if (data.startswith("set_forward:") or data.startswith("set_post:")) and current_state == RuleStates.waiting_value.state:
        ...
        await call.message.answer(t(language, "rule_updated"), reply_markup=main_menu(language))
        await state.clear()
        await call.answer()
        return
    ...
    if data.startswith("check_pair:") and current_state == CheckStates.waiting_pair.state:
        ...
        await call.message.answer(t(language, "check_done", count=len([p for p in pairs if p])), reply_markup=main_menu(language))
        await state.clear()
        await call.answer()
        return
...
async def message_router(message: Message, state: FSMContext) -> None:
    ...
    if normalized in {text.lower() for text in CANCEL_TEXTS}:
        await _cancel_flow(message, state, language)
        return
    ...
    if action and action != "language":
        ...
        if not allowed_user:
            return
    if action == "language":
        await message.answer(t(language, "language_choose"), reply_markup=language_keyboard())
        return
    if action == "help":
        await state.clear()
        await message.answer(build_tutorial(language), reply_markup=main_menu(language))
    ...
    if action == "status":
        await state.clear()
        await message.answer(await _status_text(message.from_user.id, language), reply_markup=main_menu(language))
        return
    if action == "contact":
        await state.clear()
        await message.answer(t(language, "contact"), reply_markup=main_menu(language))
        return
    if action == "add_pair":
        await state.clear()
        await _set_step(state, AddPairStates.waiting_pair_no, prompt_key="pair_no_prompt", markup_payload={"type": "main"}, remember=False)
        await message.answer(t(language, "pair_no_prompt"), reply_markup=main_menu(language))
        return
    if action == "delete_pair":
        await state.clear()
        await _set_step(state, DeletePairStates.waiting_pair_no, prompt_key="send_pair_number", markup_payload={"type": "main"}, remember=False)
        await message.answer(t(language, "send_pair_number"), reply_markup=main_menu(language))
        return
    if action == "edit_source":
        await state.clear()
        await _set_step(state, EditSourceStates.waiting_pair_no, prompt_key="send_pair_number", markup_payload={"type": "main"}, remember=False)
        await message.answer(t(language, "send_pair_number"), reply_markup=main_menu(language))
        return
    if action == "edit_target":
        await state.clear()
        await _set_step(state, EditTargetStates.waiting_pair_no, prompt_key="send_pair_number", markup_payload={"type": "main"}, remember=False)
        await message.answer(t(language, "send_pair_number"), reply_markup=main_menu(language))
        return
    if action == "keyword":
        await state.clear()
        ...
        await message.answer(t(language, "keyword_intro"), reply_markup=pair_picker("kw_pair", pairs, language))
        return
    if action == "ads":
        await state.clear()
        await _set_step(state, AdsStates.waiting_action, prompt_key="ads_intro", markup_payload={"type": "ads_actions"}, remember=False)
        await message.answer(t(language, "ads_intro"), reply_markup=ads_action_keyboard(language))
        return
    if action in {"forward_rule", "post_rule"}:
        ...
        pairs = await pair_repo.list_for_user(message.from_user.id)
        await message.answer(t(language, prompt_key), reply_markup=pair_picker("rule_pair", pairs, language))
        return
    if action == "check":
        await state.clear()
        ...
        pairs = await pair_repo.list_for_user(message.from_user.id)
        await message.answer(t(language, "choose_pair_or_all"), reply_markup=pair_picker("check_pair", pairs, language, include_all=True))
        return
    ...
    if current_state == OtpStates.waiting_otp.state:
        ...
        if status == "invalid":
            await message.answer(t(language, "otp_invalid"), reply_markup=language_keyboard())
            return
        if status == "used":
            await message.answer(t(language, "otp_used"), reply_markup=language_keyboard())
            return
        if has_old_data:
            ...
            await message.answer(t(language, "restore_choice"), reply_markup=restore_keyboard(language))
            return
        await state.clear()
        await message.answer(t(language, "otp_ok"))
        await message.answer(build_tutorial(language), reply_markup=main_menu(language))
        return
    ...
    if current_state == AddPairStates.waiting_pair_no.state:
        try:
            ...
            await message.answer(t(language, "invalid_number"))
            return
        try:
        ...
        await _set_step(state, AddPairStates.waiting_source, prompt_key="source_prompt", markup_payload={"type": "main"})
        await message.answer(t(language, "source_prompt"), reply_markup=main_menu(language))
        return

    if current_state == AddPairStates.waiting_source.state:
        await state.update_data(source_input=message.text.strip())
        await _set_step(state, AddPairStates.waiting_scan, prompt_key="scan_prompt", markup_payload={"type": "main"})
        await message.answer(t(language, "scan_prompt"), reply_markup=main_menu(language))
        return
    ...
    if current_state == AddPairStates.waiting_scan.state:
        try:
            ...
            await message.answer(t(language, "invalid_scan"))
            return
        await state.update_data(scan_count=scan_count)
        await _set_step(state, AddPairStates.waiting_target, prompt_key="target_prompt", markup_payload={"type": "main"})
        await message.answer(t(language, "target_prompt"), reply_markup=main_menu(language))
        return

    if current_state == AddPairStates.waiting_target.state:
        await state.update_data(target_input=message.text.strip())
        await _set_step(state, AddPairStates.waiting_ads, prompt_key="ads_prompt", markup_payload={"type": "main"})
        await message.answer(t(language, "ads_prompt"), reply_markup=main_menu(language))
        return
    ...
    if current_state == AddPairStates.waiting_ads.state:
        ...
        await message.answer(t(language, "rule_post_explain"), reply_markup=rule_keyboard("add_post", language))
        return
    ...
    if current_state == DeletePairStates.waiting_pair_no.state:
        ...
        if not pair or not pair.active:
            await message.answer(t(language, "pair_not_found"))
            return
        await state.update_data(pair_no=pair_no)
        await _set_step(state, DeletePairStates.waiting_confirm, prompt_key="delete_confirm", markup_payload={"type": "delete_confirm"})
        await message.answer(_pair_line(pair), reply_markup=confirm_keyboard("pair_delete", language))
        return
    ...
    if current_state == EditSourceStates.waiting_pair_no.state:
        ...
        if not pair or not pair.active:
            ...
            return
        await state.update_data(pair_no=pair_no)
        await _set_step(state, EditSourceStates.waiting_source, prompt_key="send_new_source", markup_payload={"type": "main"})
        await message.answer(t(language, "send_new_source"), reply_markup=main_menu(language))
        return

    if current_state == EditSourceStates.waiting_source.state:
        await state.update_data(source_input=message.text.strip())
        await _set_step(state, EditSourceStates.waiting_scan, prompt_key="send_scan_again", markup_payload={"type": "main"})
        await message.answer(t(language, "send_scan_again"), reply_markup=main_menu(language))
        return
    ...
    if current_state == EditSourceStates.waiting_scan.state:
        ...
        await message.answer(summary, reply_markup=confirm_keyboard("edit_source_confirm", language))
        return
    ...
    if current_state == EditTargetStates.waiting_pair_no.state:
        ...
        if not pair or not pair.active:
            ...
            return
        await state.update_data(pair_no=pair_no)
        await _set_step(state, EditTargetStates.waiting_target, prompt_key="send_new_target", markup_payload={"type": "main"})
        await message.answer(t(language, "send_new_target"), reply_markup=main_menu(language))
        return
    ...
    if current_state == EditTargetStates.waiting_target.state:
        ...
        await _set_step(state, EditTargetStates.waiting_confirm, prompt_key="edit_target_confirm", markup_payload={"type": "edit_target_confirm"})
        await message.answer(summary, reply_markup=confirm_keyboard("edit_target_confirm", language))
        return
    ...
    if current_state == KeywordStates.waiting_add_values.state:
        ...
        if not pair:
            await message.answer(t(language, "pair_not_found"), reply_markup=main_menu(language))
            await state.clear()
            return
        ...
        await message.answer(t(language, "keyword_updated"), reply_markup=main_menu(language))
        await state.clear()
        return
    ...
    if current_state == KeywordStates.waiting_clear_values.state:
        ...
        if message.text.strip().lower() == "all":
            ...
            await message.answer(t(language, "keyword_cleared"), reply_markup=main_menu(language))
            await state.clear()
            return
        values = pair_service.normalize_keywords(message.text)
    ...
    if current_state in callback_only_states:
        await message.answer(t(language, "choose_from_buttons"))
        return

    await message.answer(t(language, "unknown_menu"), reply_markup=main_menu(language))
