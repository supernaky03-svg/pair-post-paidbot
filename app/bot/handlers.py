...
async def _remove_main_menu(target: Message | CallbackQuery) -> None:
    if isinstance(target, Message):
        msg = await target.answer("⁣", reply_markup=hide_reply_keyboard())
    else:
        msg = await target.message.answer("⁣", reply_markup=hide_reply_keyboard())
    try:
        await msg.delete()
    except Exception:
        pass
...
async def _go_back(target: Message | CallbackQuery, state: FSMContext, language: str, user_id: int) -> None:
    ...
    if not history:
        await state.clear()
        await _show_step(target, state, t(language, "cancelled"), reply_markup=await _idle_markup_for_user(user_id, language), reset_panel=True)
        return
...
async def _ensure_timeout(target: Message | CallbackQuery, state: FSMContext, language: str) -> bool:
    ...
    if last_activity and (_now_ts() - float(last_activity)) > settings.flow_timeout_minutes * 60:
        await state.clear()
        await _show_step(target, state, t(language, "flow_timeout"), reply_markup=await _idle_markup_for_user(target.from_user.id, language), reset_panel=True)
        return False
...
async def _ensure_access_message(message: Message, state: FSMContext):
    ...
    await _show_step(message, state, reason, reply_markup=main_menu(_lang(user)), reset_panel=True)
    return None
...
async def _ensure_access_callback(call: CallbackQuery, state: FSMContext):
    ...
    await _show_step(call, state, reason, reply_markup=main_menu(_lang(user)), reset_panel=True)
    await call.answer()
    return None
...
async def callback_router(call: CallbackQuery, state: FSMContext) -> None:
    ...
    if data.startswith("lang:"):
        ...
        if await state.get_state() and state_data.get("current_prompt_key"):
            ...
            await _show_step(call, state, t(new_language, prompt_key, **prompt_fmt), reply_markup=markup)
        else:
...
async def message_router(message: Message, state: FSMContext) -> None:
    ...
    if current_state == EditTargetStates.waiting_target.state:
        ...
        await _show_step(message, state, summary, reply_markup=confirm_keyboard("edit_target_confirm", language))
        return
    ...
    if current_state in callback_only_states:
        await _show_step(message, state, t(language, "choose_from_buttons"))
        return
