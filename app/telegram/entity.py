...
async def resolve_source(raw: str) -> ResolvedSource:
    ...
    if invite_hash:
        try:
            ...
            return ResolvedSource(raw, build_source_key(SOURCE_KIND_PRIVATE, invite_hash), SOURCE_KIND_PRIVATE, invite_hash, invite_hash, entity, True, getattr(entity, "id", None), getattr(entity, "title", None))
...
async def resolve_target(raw: str):
    raw = raw.strip()
    if raw.lstrip("-").isdigit():
        return await safe_get_entity(int(raw))
    public_value = normalize_public_source(raw)
    if not public_value:
        raise ValidationError("Empty target.")
    return await safe_get_entity(public_value)
