...
def apply_remove_text_values(text: str, values: list[str]) -> str:
    cleaned = text or ""
...
def maybe_clean_text(pair: PairRecord, text: str) -> str:
    cleaned = apply_remove_text_values(text, pair.remove_text_values)
    if pair.remove_url_rule:
        return strip_links_preserve_text(cleaned)
...
def album_has_video(album: Iterable[Any]) -> bool:
...
def should_apply_ads(pair: PairRecord, *, has_video: bool) -> bool:
    ads = [a.strip() for a in pair.ads if a and a.strip()]
    if not ads:
        return False
    mode = pair.ads_mode or "all"
    if mode == "video_only":
        return has_video
    if mode == "non_video_only":
...
def build_single_text(pair: PairRecord, msg: Any) -> str:
    has_video = is_video_message(msg)
    original_caption = "" if pair.video_post_remove and has_video else message_text(msg)
    cleaned = maybe_clean_text(pair, original_caption)
    if should_apply_ads(pair, has_video=has_video):
        return append_ads(cleaned, pair.ads)
    return cleaned


def build_album_captions(pair: PairRecord, album: list[Any]) -> list[str]:
    has_video = album_has_video(album)
    ads_text = "\n".join(a.strip() for a in pair.ads if a and a.strip())
    captions: list[str] = []
    for index, msg in enumerate(album):
        original_caption = "" if pair.video_post_remove and has_video else message_text(msg)
        text = maybe_clean_text(pair, original_caption)
        if index == 0 and ads_text and should_apply_ads(pair, has_video=has_video):
...
def build_album_text(pair: PairRecord, album: list[Any]) -> str:
    has_video = album_has_video(album)
    source_text = "" if pair.video_post_remove and has_video else collect_album_text(album)
    cleaned = maybe_clean_text(pair, source_text)
    if should_apply_ads(pair, has_video=has_video):
        return append_ads(cleaned, pair.ads)
    return cleaned
...
def should_process_album(pair: PairRecord, album: list[Any], *, bypass_post_rule: bool = False) -> bool:
    ...
    if not pair_keyword_allows_album(pair, album):
        return False
    if pair.post_rule and not bypass_post_rule and not album_has_video(album):
        return False
    return True
